-- Final Working Migration - Handles all edge cases correctly
-- This migration creates everything in the proper sequence

-- Step 1: Drop existing tables and everything else (if they exist)
DO $$
BEGIN
    -- Drop existing tables if they exist (this will remove any data and policies)
    ...
    RAISE NOTICE 'Dropped existing tables and all related objects (policies, triggers, etc.)';
END $$;
...
-- Step 4: Create tables in correct order (dependencies first)
...
-- Step 7: Create RLS policies (now tables exist, so policies can be created)
...
-- Step 9: Create trigger to automatically create user profile
...
-- Step 11: Insert existing auth users into public.users (if any exist)
DO $$
BEGIN
    -- Only try to insert if there are auth users
...
BEGIN
    ...
    RAISE NOTICE 'Your authentication system is now ready to use!';
END $$;
